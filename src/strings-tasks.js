...
function countVowels(str) {
  const root = 'aeiouyAEIOUY';
  let result = 0;
  for (let i = 0; i < str.length; i += 1) {
    if (root.includes(str[i])) result += 1;
  }
  return result;
}
...
function isPalindrome(str) {
  const result = str.toLowerCase().match(/\w/gi);
...
function invertCase(str) {
  ...
  let result = '';
  for (let i = 0; i < str.length; i += 1) {
    result += regExp.test(str[i]) ? str[i].toLowerCase() : str[i].toUpperCase();
  }
